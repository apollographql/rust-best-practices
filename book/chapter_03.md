...
Algorithm and data flow first: repeated work, needless materialization, deep clones, serialization round trips, lock contention, unbounded queues. Then micro-level. Guard complex optimizations with a differential oracle ([testing](chapter_05.md)).
...
- Monomorphization trades runtime for code size and compile time ([dispatch](chapter_06.md)).

Re-measure the whole workload after a change, including other dimensions. Keep the simplest version that meets the target.
