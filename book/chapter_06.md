# Chapter 6 — Abstraction and Dispatch

**Domain boundary before dispatch mechanism.** Nothing varies → concrete type. Closed set → enum + exhaustive match. Open family or downstream extension → trait. No trait per struct or per hypothetical second impl; for test seams see [testing](chapter_05.md).
...
- Expose the smallest contract consumers need; use installed crates' extension points over parallel adapter layers.
