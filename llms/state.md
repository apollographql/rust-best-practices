...
- Deserialization: wire type + `#[serde(try_from = "Raw")]` (+ `into = "Raw"` to serialize). A plain `#[derive(Deserialize)]` on a domain type bypasses its constructor.
...
- Let state carry its data: `Connected { socket }`, not `state: State` + `socket: Option<Socket>`. Existing types may already be the state: an open `File` is the opened state; a wrapper must add a real guarantee.
...
- Builders are an API choice, not a typestate showcase. Few required fields → constructor args; substantial builders → `bon` or `typed-builder`.
